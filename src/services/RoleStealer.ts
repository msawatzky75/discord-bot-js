...
export class RoleStealer {
	...
	constructor(
		@inject(TYPES.StealableRoles) roles: Snowflake[]
	) {
		this.roles = roles;
	}
	...
	handle(message: Message): Promise<Message | Message[]> {
		...
		if (this.roles.length > 0 && message.guild) {
			...
			const vicitms = message.guild.members.cache
				.filter(member => member.roles.cache.hasAny(...roles.map(role => role.id)));
			...
			return message.channel.send(`${message.author} has stolen the ${roles.map(role => role.name).join(", ")} role(s) from ${vicitms.size} user(s)`);
		}
